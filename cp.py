...
# -*- coding: utf-8 -*-
"""
Created on Mon May 13 15:14:09 2019
...
def cp(tensor, rank,W=None, n_iter_max=100, tol=1e-8,orthogonalise=False, random_state=None, verbose=False, return_errors=False):
    if orthogonalise and not isinstance(orthogonalise, int):
        orthogonalise = n_iter_max

    factors = initialize_factors(tensor, rank, random_state=random_state)
    rec_errors = []
    delta_X=(1-W)*kruskal_to_tensor(factors)   
    X=tensor+delta_X #填充X
    norm_x=tl.norm(tensor,2)
    norm_delta=tl.norm(delta_X,2)
    for iteration in range(n_iter_max):
        if orthogonalise and iteration <= orthogonalise:
            factor = [tl.qr(factor)[0] for factor in factors]
            
        for mode in range(tl.ndim(tensor)):
            pseudo_inverse = tl.tensor(np.ones((rank, rank)), **tl.context(X))
            for i, factor in enumerate(factors):
                if i != mode:
                    pseudo_inverse = pseudo_inverse*tl.dot(tl.transpose(factor), factor)
            factor = tl.dot(unfold(X, mode), khatri_rao(factors, skip_matrix=mode))
            factor = tl.transpose(tl.solve(tl.transpose(pseudo_inverse), tl.transpose(factor)))
            factors[mode] = factor

        if tol:
            tensor_error=tl.norm(tensor-kruskal_to_tensor(factors), 2)/norm_x
            rec_error = tl.norm(delta_X-(1-W)*kruskal_to_tensor(factors), 2)/norm_delta
            delta_X=(1-W)*kruskal_to_tensor(factors)
            norm_delta=tl.norm(delta_X,2)
            X=tensor+delta_X
